...
function SolarisLib:New(Config)
	...
	function SettingsConstructor()
		...
		task.spawn(function()
			pcall(function()
				while task.wait() do
					if not SFrame:FindFirstChild("TopBar") then continue end;
		...
		function Settings:Tab(text)
			...
			Tab.MouseButton1Click:Connect(function()
				for i,v in next, SFrame.TabHolder:GetChildren() do
					if v.Name == "Tab" then v.TextTransparency = 0.4 end
				end
			...
			function TabHold:BindSetting(title, desc, def, path)
				...
				UserInputService.InputBegan:Connect(function(Input)
					if UserInputService:GetFocusedTextBox() or not closebindbinding then return end
					local Key;
					pcall(function() if not KeyCheck(BlacklistedKeys, Input.KeyCode) then Key = Input.KeyCode end end)
					pcall(function() if KeyCheck(WhitelistedMouse, Input.UserInputType) and not Key then Key = Input.UserInputType end end)
					Key = Key or value
					SetValue(Key)
				end)
				...
				task.spawn(function()
					pcall(function()
						while task.wait() do
							if not Bind:FindFirstChild("Desc") then continue end;
			...
			function TabHold:Dropdown(title, desc, list, def, path)
				...
				task.spawn(function()
					pcall(function()
						while task.wait() do
							if not Dropdown:FindFirstChild("Main") then continue end;
	...
	local function OpenTabMenu() TweenService:Create(MFrame.TabMenu,TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),{Position = UDim2.new(0,0,0,0)}):Play() end
	local function CloseTabMenu() TweenService:Create(MFrame.TabMenu,TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),{Position = UDim2.new(-0.37, -8, 0, 0)}):Play() end 

	MFrame.TopBar.TabListBtn.MouseButton1Click:Connect(OpenTabMenu)
	MFrame.TabMenu.Menu.Top.MenuCloseBtn.MouseButton1Click:Connect(CloseTabMenu)
	...
	function SolarisLib:LoadCfg(cfg)
		...
		table.foreach(content, function(a,b)
			if SolarisLib.Flags[a] then
				task.spawn(function() SolarisLib.Flags[a]:Set(b) end)
			else
	...
	function TabHolder:Tab(text)
        ...
        return Button
			...
			function ItemHold:Bind(text, preset, holdmode, flag, callback)
				...
				BindFrame.InputEnded:Connect(function(Input, Typing)
					if Input.UserInputType == Enum.UserInputType.MouseButton1 then
